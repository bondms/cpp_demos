...
bool isprint(char ch) { return ch >= 0x20 && ch <= 0x7E; }
...
std::string Escaped(char raw) {
  ...
  oss << "x" << std::hex << std::setfill('0') << std::setw(2) << std::uppercase
...
std::string Escaped(const std::string &raw) {
  ...
  while (true) {
    ...
    // Replace the non-printable character with a '\' and insert the escaped
