...
auto make_time_point(int year, int month, int day, int hour, int minute,
...
class LoggerTestFixture : public testing::Test {
  void SetUp() override { std::filesystem::create_directory(temp_test_path_); }
...
protected:
  std::filesystem::path temp_test_path_{std::filesystem::temp_directory_path() /
                                        OS_TEXT("LoggerTest")};
...
TEST_F(LoggerTestFixture, MessageTimeStamp) {
  EXPECT_EQ(
      ...
      Logger::MessageTimeStamp(make_time_point(2019, 12, 14, 13, 21, 12, 123)));
  EXPECT_EQ("2019-01-02T01:02:03.003Z",
            Logger::MessageTimeStamp(make_time_point(2019, 1, 2, 1, 2, 3, 3)));
}

TEST_F(LoggerTestFixture, FileNameTimeStamp) {
  EXPECT_EQ(OS_TEXT("20191214_132112"),
            Logger::FileNameTimeStamp(
                make_time_point(2019, 12, 14, 13, 21, 12, 123)));
  EXPECT_EQ(OS_TEXT("20190102_010203"),
...
TEST_F(LoggerTestFixture, LOG_messages_append) {
  ...
  ASSERT_EQ(2, lines.size());
  {
    ...
    if (!std::regex_match(lines[0], re)) {
      ADD_FAILURE() << "First logged message did not match expected: "
                    << lines[0];
    }
  }
  {
