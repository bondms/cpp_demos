...
template <typename JobData> class Completor {
  Sync<JobData> &sync_;
      ...
      while (true) {
...

        {
          ...
          if (completed) {
            ...
            auto &containerItemRef{sync_.pool.find_if(
                [&](const typename Pool<JobData>::ContainerItem
                        &containerItem) {
                  return jobMatchFunction_(containerItem.data, jobData);
                })};
