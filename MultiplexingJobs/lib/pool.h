...
template <typename JobData> class Pool {
public:
  struct ContainerItem {
    ContainerItem(const JobData &jobData, const State jobState)
        : data{jobData}, state{jobState} {}
    JobData data{};
    State state{};
  };
...
public:
  ...
  bool availableToStart() const { return container_.end() != nextToStart_; }
  ...
  template <typename Pred> ContainerItem &find_if(Pred pred) {
    const auto it{std::find_if(container_.begin(), container_.end(), pred)};
