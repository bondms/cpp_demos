...
template <typename JobData> class Multiplexor {
  Sync<JobData> sync_{};
...
public:
  Multiplexor(typename Functions<JobData>::InitiateFunction initiateFunction,
      ...
      : timeout_{timeout}, initiator_{sync_, initiateFunction},
  ...
  void processJob(JobData &jobData) {
...

    {
      ...
      auto pred = [&]() {
        return sync_.quit || (!sync_.error.empty()) ||
