...
class Event {
public:
  enum class State { nonSignalled, signalled };

  enum class Mode { manualReset, autoReset };
...
public:
  ...
  bool WaitFor(const std::chrono::duration<Rep, Period> &relTime) {
    ...
    return condition_variable_.wait_for(lock, relTime,
  ...
  bool WaitUntil(const std::chrono::time_point<Clock, Duration> &timeoutTime) {
    std::unique_lock<std::mutex> lock{mutex_};
    return condition_variable_.wait_until(lock, timeoutTime,
                                          [&] { return WaitPredicate(); });
  }
};
