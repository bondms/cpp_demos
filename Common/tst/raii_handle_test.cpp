...
public:
  void free() noexcept { freed_ = true; };

  bool freed() const noexcept { return freed_; }

  void reset() noexcept { freed_ = false; }
};
...
class RaiiHandleTestFixture : public testing::Test {
public:
  MockClass mockClass_{};
  void SetUp() override { mockClass_.reset(); }
  void mockAllocate(MockHandle *pMockHandle) { *pMockHandle = &mockClass_; }
};
